...
//                                  "    vColor = gl_Position.wyxz + vec4(1.0f, 1.0f, 1.0f, 1.0f);\n"
//                                  "    gl_PointSize = 10.0f;\n"
//                                  "}\n\0";
// // 片段着色器1 测试in vec4 vColor
// const char *fragmentShaderSource1 = "#version 330 core\n"
//                                     "out vec4 FragColor;\n"
//                                     "in vec4 vColor;\n" // 必须名称类型都相同vec4 vColor
//                                     "void main()\n"
//                                     "{\n"
//                                     "    FragColor = vColor/2;\n"
//                                     "}\n\0";

// // 片段着色器   测试 uniform vec4 ourColor
...
//         float Value1 = (sin(timeValue) / 2.0f) + 0.5f;
//         float Value2 = (cos(timeValue) / 2.0f) + 0.5f;
//         float Value3 = (tan(timeValue) / 2.0f) + 0.5f;
//         int ourColorLocation = glGetUniformLocation(shaderPrograms[1], "ourColor"); // 获取uniform vec4 ourColor的地址
//         // 更新一个uniform之前你必须先使用程序（调用glUseProgram)，查询uniform地址不要求
//         glUniform4f(ourColorLocation, Value1, Value2, Value3, 1.0f); // 改变ourColor的值
