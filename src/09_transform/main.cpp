...
int main(int agrc, char *argv[])
{
    ...
    /*
        // 初始化向量
        glm::vec4 postion = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
        cout << postion.x << ", " << postion.y << ", " << postion.z << endl;
        // 初始化矩阵
        glm::mat4 trans = glm::mat4(1.0f); // 单位矩阵

        // 平移
        trans = glm::translate(trans, glm::vec3(1.0f, 0.0f, 0.0f)); // x轴向右平移一个单位
        // postion = trans * postion;
        // cout << "平移：" << postion.x << ", " << postion.y << ", " << postion.z << endl; // 2 1 1

        // 缩放
        trans = glm::scale(trans, glm::vec3(2.0f, 3.0f, 4.0f));
        postion = trans * postion;
        cout << "缩放：" << postion.x << ", " << postion.y << ", " << postion.z << endl; // 5 3 4

        // 旋转
        trans = glm::rotate(trans, glm::radians(45.0f), glm::vec3(0.0f, 0.0f, 1.0f)); // 第三个参数表示绕哪个向量进行旋转
        postion = trans * postion;
        cout << "旋转：" << postion.x << ", " << postion.y << ", " << postion.z << endl;
    ...
    trans = glm::rotate(trans, glm::radians(45.0f), glm::vec3(0.0f, 0.0f, 1.0f)); // translate * rotate
    trans = glm::scale(trans, glm::vec3(0.5f, 0.5f, 0.5f));                       // translate * rotate * scale
    // 将矩阵传送给着色器
    ...
    while (!glfwWindowShouldClose(window)) // 窗口是否关闭
    {
        ...
        float Value = glfwGetTime(); // 获取时间
        // ourShader.setFloat("angleValue", Value);

        // 记得每次都将trans单位化,不然每次循环的数值会叠加起来,导致图像超出范围
        trans = glm::mat4(1.0f);
        trans = glm::translate(trans, glm::vec3(sin(Value), cos(Value), 0.0f));   // translate
        trans = glm::rotate(trans, Value, glm::vec3(0.0f, 0.0f, 1.0f));           // translate * rotate
        trans = glm::scale(trans, glm::vec3(sin(Value), sin(Value), sin(Value))); // translate * rotate * scale

        // trans = glm::rotate(trans, Value, glm::vec3(0.0f, 0.0f, 1.0f)); // rotate
        // trans = glm::translate(trans, glm::vec3(0.5f, 0.0f, 0.0f));     // rotate*translate
        // trans = glm::scale(trans, glm::vec3(0.5f, 0.5f, 0.5f));         // rotate*translate*scale
        // 将矩阵传送给着色器
        ourShader.setMat4("transform", trans); // translate * rotate * scale * vec4
